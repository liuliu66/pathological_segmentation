...
from .utils import rotate_image, calculate_rotation, crop_around_center, largest_rotated_rect
from ..registry import PIPELINES


@PIPELINES.register_module
class RandomBlur(object):
    def __init__(self, blur_limit=7, blur_ratio=0.5):
        self.blur_limit = blur_limit
        self.blur_ratio = blur_ratio

    def __call__(self, results):
        blur_operator = albumentations.Blur(blur_limit=self.blur_limit, always_apply=False, p=self.blur_ratio)
        results['img'] = blur_operator(image=results['img'])['image']
...
class Resize(object):
    ...
    def _resize_img_nocs(self, results):
        if self.keep_ratio:
            img, scale_factor = mmcv.imrescale(
                results['img'], results['scale'], return_scale=True)
            depth, scale_factor = mmcv.imrescale(
                results['depth'], results['scale'], return_scale=True)
        else:
            img, w_scale, h_scale = mmcv.imresize(
                results['img'], results['scale'], return_scale=True)
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
            depth, w_scale, h_scale = mmcv.imresize(
                results['depth'], results['scale'], return_scale=True)
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
    ...
    def _resize_masks2(self, results):
        for key in results.get('mask_fields', []):
            if results[key] is None:
                continue
            mask_scale = (results['img_shape'][0] / results[key].shape[0],
                           results['img_shape'][1] / results[key].shape[1],
                           1)
            masks = scipy.ndimage.zoom(results[key], mask_scale, order=0)

            results[key] = masks

    def _resize_coords(self, results):
        for key in results.get('coord_fields', []):
            if results[key] is None:
                continue
            coord_scale = (results['img_shape'][0] / results[key].shape[0],
                           results['img_shape'][1] / results[key].shape[1],
                           1, 1)
            coords = scipy.ndimage.zoom(results[key], coord_scale, order=0)
    ...
    def __call__(self, results):
        ...
        if self.resize_type == 'rgb':
            self._resize_img(results)
            self._resize_bboxes(results)
            self._resize_masks(results)
        else:
            self._resize_img_nocs(results)
            self._resize_masks2(results)
...
class Pad(object):
    ...
    def _pad_coords(self, results):
        pad_shape = results['pad_shape'][:2]
        for key in results.get('coord_fields', []):
            padded_coords = [
                mmcv.impad(coord, pad_shape, pad_val=self.pad_val)
                for coord in results[key]
            ]
            results[key] = np.stack(padded_coords, axis=0)

    def __call__(self, results):
        self._pad_img(results)
        self._pad_masks(results)
        self._pad_coords(results)
        return results
...
class Normalize(object):
    ...
    def __init__(self, mean, std, to_rgb=True, is_nocs=False):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        self.to_rgb = to_rgb
        self.is_nocs = is_nocs
...
class RandomGammaCorrection(object):
    """Apply gamma correction by skimage.exposure.adjust_gamma.

    Args:
        gamma_min (float, optional): The random gamma minimum.
        gain_min (float, optional): The random gain minimum
    """

    def __init__(self, gamma_min=0.8, gain_min=0.8):
        self.gamma_min = gamma_min
        self.gain_min = gain_min

    def __call__(self, results):
        gamma = np.random.uniform(self.gamma_min, 1)
        gain = np.random.uniform(self.gain_min, 1)
        results['img'] = exposure.adjust_gamma(results['img'], gamma, gain)
        return results
...
class RandomRotate(object):
    """Rotate augmentation for input data.

    Args:
        rotate_min (float, optional): The random rotate degree minimum.
        rotate_max (float, optional): The random rotate degree maximum.
    """

    def __init__(self, rotate_min=-5, rotate_max=5):
        self.rotate_min = rotate_min
        self.rotate_max = rotate_max

    @staticmethod
    def rotate_and_crop_images(results, rotate_degree):

        image_height, image_width = results['img'].shape[0:2]
        new_w, new_h, affine_mat = calculate_rotation(results['img'].shape[0:2][::-1], rotate_degree)

        results['img'] = rotate_image(results['img'], new_w, new_h, affine_mat, cv2.INTER_LINEAR)
        results['gt_masks'] = rotate_image(results['gt_masks'], new_w, new_h, affine_mat, cv2.INTER_NEAREST)
        ...
        results['img'] = crop_around_center(results['img'], *rect)
        results['gt_masks'] = crop_around_center(results['gt_masks'], *rect)

        image_rotated_cropped = cv2.resize(results['img'], (image_width, image_height),
                                           interpolation=cv2.INTER_LINEAR)
        mask_rotated_cropped = cv2.resize(results['gt_masks'], (image_width, image_height),
                                          interpolation=cv2.INTER_NEAREST)

        if results['gt_coords'] is not None:
            results['gt_coords'] = rotate_image(results['gt_coords'], new_w, new_h, affine_mat, cv2.INTER_NEAREST)
            results['gt_coords'] = crop_around_center(results['gt_coords'], *rect)
            results['gt_coords'] = cv2.resize(results['gt_coords'], (image_width, image_height),
                                               interpolation=cv2.INTER_NEAREST)
        return results

    def __call__(self, results):
        rotate_degree = np.random.uniform(-5, 5)
        results = self.rotate_and_crop_images(results, rotate_degree)
