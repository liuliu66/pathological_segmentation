...
class SelectiveFCNMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(SelectiveFCNMaskHead, self).__init__()
        ...
        for level, roi_feat in enumerate(x):
            ...
            if self.upsample is not None:
                ...
                if self.upsample_method == 'deconv':
                    r = self.relu(r)
            r = r * weights[level].unsqueeze(1).unsqueeze(2).unsqueeze(3)
            result.append(r)
        result = sum(result)
        mask_pred = self.conv_logits(result)
        return mask_pred
