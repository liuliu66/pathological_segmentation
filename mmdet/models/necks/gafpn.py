...
class GaFPN(nn.Module):
    ...
    def __init__(self,
        ...
        for channel in self.in_channels:
            self.ca_modules.append(nn.Sequential(
        ...
        for channel in self.in_channels:
            self.sa_modules.append(nn.Sequential(
                nn.Conv2d(channel, 1, kernel_size=1, padding=0),
                nn.BatchNorm2d(1),
                nn.ReLU(inplace=True),
                nn.Conv2d(1, 1, kernel_size=5, padding=2, dilation=1),
                nn.BatchNorm2d(1),
                nn.Sigmoid()
            ))
    ...
    def forward(self, inputs):
        ...
        # add global activation to inputs
        inputs_ga = []
        for i, input in enumerate(inputs):
            input_ca = self.ca_modules[i](input) * input
            input_sa = self.sa_modules[i](input) * input
            inputs_ga.append(input_ca + input_sa)
        inputs_ga = tuple(inputs_ga)

        # build laterals
        laterals = [
            lateral_conv(inputs_ga[i + self.start_level])
        ...
        if self.num_outs > len(outs):
            ...
            # add conv layers on top of original feature maps (RetinaNet)
                ...
                if self.extra_convs_on_inputs:
                    orig = inputs_ga[self.backbone_end_level - 1]
