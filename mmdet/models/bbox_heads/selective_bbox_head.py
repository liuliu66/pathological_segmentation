...
class SelectiveFCBBoxHead(ConvFCBBoxHead):

    def __init__(self, num_fcs=-1, fc_out_channels=1024, num_scales=4, *args, **kwargs):
        super(SelectiveFCBBoxHead, self).__init__(
            num_shared_convs=0,
        ...
        #for _ in range(self.num_scales):
            #self.fc6_list.append(
            #    nn.Linear(in_channels, fc_out_channels)
            #)
            #self.fc7_list.append(
            #    nn.Linear(fc_out_channels, fc_out_channels)
            #)
        #self.fc6_list = nn.ModuleList(self.fc6_list)
        #self.fc7_list = nn.ModuleList(self.fc7_list)
    ...
    def init_weights(self):
        ...
        for module_list in [self.fc6_list, self.fc7_list]:
    ...
    def forward(self, x, **kwargs):
        ...
        #for level, roi_feat in enumerate(x):
            #feature = roi_feat.view(roi_feat.size(0), -1)
            #feature = self.relu(self.fc6_list[level](feature))
            #feature = self.relu(self.fc7_list[level](feature))
