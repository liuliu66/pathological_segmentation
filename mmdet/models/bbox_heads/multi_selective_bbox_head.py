...
class MultiSelectiveFCBBoxHead(ConvFCBBoxHead):

    def __init__(self, num_fcs=2, fc_out_channels=1024, num_scales=4, *args, **kwargs):
        super(MultiSelectiveFCBBoxHead, self).__init__(
            num_shared_convs=0,
        ...
        self.shared_convs2, self.shared_fcs2, last_layer_dim = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, self.in_channels,
                True)

        self.shared_convs3, self.shared_fcs3, last_layer_dim = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, self.in_channels,
                True)
    ...
    def init_weights(self):
        ...
        for module_list in [self.shared_fcs2, self.shared_fcs3]:
